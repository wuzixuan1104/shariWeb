...
$(function() {
  ...
  window.oaips = {
    ...
    show: function(index, $obj, da, fromURL) {
      if (isNaN(index) || !window.oaips.$pswp || !window.oaips.$conter)
        return;
      ...
      g.init(function(pvid) {
        if (!(window.oaips.callPvfunc && (typeof window.oaips.callPvfunc === 'function') && pvid.length && (pvid.split('-').length == 2)))
          return false;
        window.oaips.callPvfunc(pvid.split('-')[0], pvid.split('-')[1]);
      });
  ...
  $('a[data-method="delete"]').click(function() {
    return !confirm('確定要刪除？') ? false : true;
  });
  ...
  $('table.list').each(function() {
    ...
    $(this).find('tbody').append(
      $('<tr />').append(
        $('<td />').attr('colspan', $(this).find('thead > tr > th').length)));
  });
  ...
  $('.oaips').each(function() {
    ...
    var $oaip = $(this).find('img').map(function() {
      ...
      if ($(this).attr('data-pvid') !== undefined)
        $div.attr('data-pvid', $(this).attr('data-pvid'));
      
      if ($(this).attr('data-ori') !== undefined)
        $div.attr('data-ori', $(this).attr('data-ori'));
    ...
    if (typeof $.fn.imgLiquid !== 'undefined')
      $oaip.imgLiquid({ verticalAlign:'center' });
  ...
  if (typeof $.fn.OAdropUploadImg !== 'undefined') {
    ...
    mutiImg = function($obj) {
      ...
      $obj.on('change', '.drop-img > input[type="file"]', function() {
        ...
        if ($obj.find('>.drop-img').last().hasClass('no'))
          return;
  ...
  window.oaGmap = {
    ...
    loaded: false,
    init: function() {
      if (window.oaGmap.loaded)
        return false;

      window.oaGmap.loaded = true;
      window.oaGmap.funcs.forEach(function(t) {
        t();
      });
    },
    runFuncs: function() {
